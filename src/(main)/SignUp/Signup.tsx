...
const [emailLoading, setEmailLoading] = useState(false);
  ...
  <div className="w-full max-w-[550px] bg-white rounded-3xl shadow-2xl overflow-hidden">
    <div className="flex w-full">
      <div onClick={()=>setToggleCards(true)} className="w-1/2 bg-[#2B6777] text-white text-center py-5 font-semibold text-lg border-r border-slate-400">
        PAN CARD
      </div>
      <div onClick={()=>setToggleCards(false)} className="w-1/2 bg-[#2b6777de] text-white text-center py-5 font-semibold text-lg">
        EMAIL
      </div>
    </div>
    {
        toggleCard?<div className="px-14 py-12">
      ...
      <div className="flex flex-col gap-4">
        ...
        <input
          ...
          className="py-3 px-6 rounded-full border-[#2B6777] text-[#2B6777] border uppercase tracking-widest text-lg
                     outline-0"/>
   <button
  disabled={panLoading}
  onClick={() => {
